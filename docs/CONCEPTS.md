...
### 어댑터(Adapter) 패턴
TMDB·iTunes·카카오는 응답 형태가 모두 다르다. 이를 **하나의 공통 카드 형태**(`{ id, type, title, subtitle, description, image, link, … }`)로 바꿔, 화면 컴포넌트가 출처를 몰라도 되게 한다.
...
→ [src/services/tmdb.js](../src/services/tmdb.js), [kakaoBooks.js](../src/services/kakaoBooks.js)
