/*
  ...
  - TMDB(영화) / Last.fm·iTunes(음악) / 카카오(도서) 등 여러 서비스가 똑같은
    fetch+에러처리+JSON파싱을 반복하지 않도록 이 한 곳에 모았다.
...
/*
  fetchJson — 주어진 URL로 요청을 보내고, 응답을 JSON 객체로 돌려준다.
    입력: url(요청할 주소 문자열), errorMessage(실패 시 보여줄 한국어 메시지),
          options(선택) — fetch에 그대로 넘기는 설정 객체. 예: { headers: { Authorization: '...' } }
    반환: Promise<객체> — 파싱된 JSON 데이터
    실패: 응답이 비정상이면 errorMessage를 담은 에러를 던진다(throw).

  ● headers(헤더)란?
    - 요청에 함께 보내는 "부가 정보표". 인증 토큰(Authorization)처럼 URL에 넣기 곤란한 값을 담는다.
    - 카카오 책검색처럼 "Authorization: KakaoAK 키" 헤더가 필요한 API를 위해 options를 받도록 했다.
    - options를 안 넘기면 기존처럼 평범한 GET 요청이 된다(기존 호출부 호환).
*/
